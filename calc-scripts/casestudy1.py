...
#
testdir = '/home/calvin/graspy/calc-outputs/casestudy1'
starting_over = False
if starting_over:
    initialize(workdir=testdir,rm = False)
...
# Split into odd3.c, odd4.c, odd5.c, odd6.c
    odd_exp_split = Rcsfsplit(calc_name = 'odd',nsplit=4,splitorbs = [3,4,5,6])
...
#Generate even CSF expansions 2.1 for 2s2p(2)
    ...
    even_2s2p2_exp.execute(workdir = testdir)

#Split into even3.c, even4.c, even5.c, even6.c
    even_2s2p2_exp_split = Rcsfsplit(calc_name = 'even',nsplit = 4, splitorbs =[3,4,5,6])
...
#copy odd2.c to rcsf.inp
    ...
    init_est_odd = [Rangular(), #Get initial estimates of wave functions
            ...
            Rmcdhf([[1],[1]],orbs = ['*'],specorbs = ['*'], runs = 100, weighting_method = 'Standard'),
...
#copy even2.c to rcsf.inp
    ...
    inti_est_even = [Rangular(),
            ...
            Rmcdhf([[1,2,3],[1,2,3],[1,2]],orbs = ['*'],specorbs = ['*'], runs = 100, weighting_method = 'Standard'),
            # Perform self-consistent field calculations
...
#Get results for odd n=3,4,5,6
    for n in [3,4,5,6]:
            ...
            print('Estimating wavefunctions from n={m} calculations')
            print('Calculating angular integrals and doing MCDHF with n={n}')
            # Get angular data
            Rangular().execute(workdir = testdir)
            calculations_odd = [
                    Rwfnestimate(orbdict = {'*':f'odd{m}.w'},fallback = 'Thomas-Fermi'), #how to represent multiple files with differing numbers?
                    # Get initial estimates of wave functions
                    Rmcdhf(asfidx = [[1],[1]],orbs = [f'{n}*'],specorbs = [], runs = 100, weighting_method = 'Standard'),
            ...
            LSJ_coupling_odd = JJtoLSJ(calc_name= f'odd{n}',use_ci = False, unique = True).execute(workdir = testdir)
...
for n in [3,4,5,6]:
    ...
    calculations_even = [Rangular(),
            ...
            # Get initial estimates of wave functions
            Rmcdhf(asfidx = [[1,2,3],[1,2,3],[1,2]],orbs = [f'{n}*'],specorbs = [], runs = 100, weighting_method = 'Standard'),
            # Perform self-consistent field calculations
            #Output = outodd_rmcdhf_${n}
            Rsave(f'even{n}')]
    ...
    # transform to LSJ-coupling
    Transform_LSJ_even = JJtoLSJ(calc_name= f'even{n}',use_ci = False, unique = True)
