...
odd_mr = Rcsfgenerate(core='None',ordering = 'Default',
            csflist=['1s(2,i)2s(2,i)2p(1,i)',
                     '1s(2,i)2p(3,i)'],
            active_set=[2,2],
            jlower=1,jhigher=3,exc=0, write_csf= 'odd.c')
            # rsfgenerate EOF3 -> copy rcsf.out to odd.c
...
# rcsfsplit each subshell into seperate file
...
even_2s2p2_mr = Rcsfgenerate(core='None',ordering = 'Default',
            ...
            active_set=[2,2],
            jlower=1,jhigher=5,exc=0, write_csf= 'even.c')
...
# take commands from ./sh_intial -> copy file rcsf.out to even2.c -> rangular -> rwfnestimate -> rncdhf -> rsave

nuclear_data = Rnucleus(Z = 42, A = 96, neutralMass = 96, I = 1, NDM = 1,NQM = 1)
nuclear_data.execute(workdir = testdir)

original = r'workdir/odd2.c'
target = r'workdir/rcsf.inp'
