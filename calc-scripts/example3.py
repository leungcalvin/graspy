...
# 1) Generate a multireference consisting of the 2s2 2p3 and 2p5 configurations
ref_2s = Rcsfgenerate(core='None',ordering = 'Default',
            csflist=['2s(2,i)2p(3,i)'],
            activeset=[2],
            jlower=1,jhigher=1,exc=0,write_csf= 'rcsfmr.inp')
print("laasya - ref_2s=",ref_2s)
...
#QUIT HERE, RCSF.INP AND RCSFMR.INP ARE BLANK, MUST DEBUG BEFORE PROCEEDING
sys.exit(1)
# 2) Perform a SCF procedure to solve for the 2s,2p orbitals.
MR_DHF =[
...
# 4) Solve for the n=3 correlation orbitals, using orbitals generated from 2s22p3_2p5_DF.w above.
indices_2S = [[1]]
CAS_2S = [
        Rangular(),
        Rwfnestimate(orbdict = {'*':'2s22p3_2p5_DF'},fallback = 'Thomas-Fermi'),
...
CI_2P = [
        Rci(calcname='2p5_3',
                ...
                largestn = 3,
                asfidx = indices_2P),
        JJtoLSJ(calcname= '2p5_3',useCI = True, unique = True),
        ]
[cmd.execute(workdir = testdir) for cmd in CI_2P]

# 9) Calculate transitions.
transitions_2P = [
        Rhfs(calcname = '2p5_3',useCI=True),
        Rbiotransform(useCI=True,calcname_initial = '2s22p3_2p5_3',calcname_final = '2p5_3', transform_all = True),
        Rtransition(useCI=True,calcname_initial = '2s22p3_2p5_3',calcname_final = '2p5_3',transition_spec = ['E1'])]
